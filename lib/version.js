...
function checkGit(data, cb) {
  ...
  var gitEnvOptions = {env: process.env}

  // check for git
  git.whichAndExec(args, gitEnvOptions, function (er, stdout) {
    ...
    write(data, function (er) {
      ...
      var message = npm.config.get("message").replace(/%s/g, data.version)
        ...
        , flag = sign ? "-sm" : "-am"
        , shr = npm.config.get("version-with-shrinkwrap")
      chain([
          function (cb) { if (shr) npm.commands.shrinkwrap([], cb); else cb(); }
          , git.chainableExec([ "add", "package.json" ], gitEnvOptions)
          , fs.existsSync("npm-shrinkwrap.json") ? git.chainableExec([ "add", "npm-shrinkwrap.json" ], gitEnvOptions) : function (cb) { cb();}
          , git.chainableExec([ "commit", "-m", message ], gitEnvOptions)
          , sign && function (cb) { npm.spinner.stop(), cb(); }
          , git.chainableExec([ "tag", "v" + data.version, flag, message ], gitEnvOptions)
        ], cb
      )
    })
  })
